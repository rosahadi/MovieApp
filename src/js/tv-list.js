...
const apiUrl = `https://api.themoviedb.org/3/tv/${list}?api_key=${API_KEY}&page=${currentPage}`;

fetchDataFromServer(apiUrl, function ({ results: movieList, total_pages }) {
  totalPages = total_pages;

  for (const movie of movieList) {
    const movieCard = createCard(movie, 'name', 'first_air_date', 'tv');
...
loadMore.addEventListener('click', function () {
  ...
  fetchDataFromServer(
    `https://api.themoviedb.org/3/tv/${list}?api_key=${API_KEY}&page=${currentPage}`,
    ({ results: movieList }) => {
